<?php
    ...
    $query_ware = mysqli_query($conn, "SELECT * FROM Towary WHERE ID_towaru = ".$_POST['ware_id'].";");

    $result_ware = mysqli_fetch_all($query_ware, MYSQLI_ASSOC);
...
<header class="menuBarDev p-3" style="background-color: rgb(88,101,242);">
  ...
  <main class="container">
    <h1>Edytuj towar</h1>
    <div class="container">
      <form method="POST" action="update_ware.php">
        <div class="form-group">
          <label for="nameInput">Nazwa</label>
          <input type="text" class="form-control" id="nameInput" name="nameInput" value="<?php echo $result_ware[0]['nazwa']; ?>" maxlength="35"/>
        </div>
        <div class="form-group">
          <label for="priceInput">Cena</label>
          <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="priceInput" name="priceInput" value="<?php echo $result_ware[0]['cena']; ?>" maxlength="5"/>
        </div>
        <div class="form-group">
          <label for="typeInput">Rodzaj</label>
          <input type="text" class="form-control" id="typeInput" name="typeInput" value="<?php echo $result_ware[0]['rodzaj']; ?>" maxlength="25" />
        </div>
        <div class="form-group">
          <label for="codeInput">Kod</label>
          <input type="text" class="form-control" id="codeInput" name="codeInput" value="<?php echo $result_ware[0]['kod']; ?>" maxlength="12"/>
        <div class="form-group">
          <label for="descriptionInput">Opis</label>
          <input type="text" class="form-control" id="descriptionInput" name="descriptionInput" value="<?php echo $result_ware[0]['opis']; ?>" maxlength="100"/>
        </div>
        <div class="form-group">
          <label for="taxInput">% Vat</label>
          <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="taxInput" name="taxInput" value="<?php echo $result_ware[0]['vat']; ?>" maxlength="2"/>
        </div>
        <input type="hidden" name="wareID" value="<?php echo $_POST['ware_id']; ?>" />
