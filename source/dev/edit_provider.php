<?php
    ...
    $query_provider = mysqli_query($conn, "SELECT * FROM Dostawcy WHERE ID_dostawcy = ".$_POST['provider_id'].";");
    $query_address = mysqli_query($conn, "SELECT * FROM Adresy WHERE ID_adresu = ".$_POST['address_id'].";");

    $result_provider = mysqli_fetch_all($query_provider, MYSQLI_ASSOC);
...
<head>
    ...
    <title>GBS - Edycja dostawcy</title>
...
<header class="menuBarDev p-3" style="background-color: rgb(88,101,242);">
    ...
    <div class="h1">Edytuj dostawcę</div>
        <form method="POST" action="update_provider.php">
            <div class="form-group">
                <label for="nameInput">Nazwa</label>
                <input type="text" class="form-control" id="nameInput" name="nameInput" value="<?php echo $result_provider[0]['nazwa']; ?>" maxlength="50" />
            </div>
            <div class="form-group">
                <label for="phoneInput">Numer telefonu</label>
                <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="phoneInput" name="phoneInput" value="<?php echo $result_provider[0]['telefon']; ?>" maxlength="12"/>
            </div>
            <div class="form-group">
                <label for="emailInput">E-Mail</label>
                <input type="text" class="form-control" id="emailInput" name="emailInput" value="<?php echo $result_provider[0]['email']; ?>" maxlength="30"/>
            </div>
            <div class="form-group">
            ...
            <input type="hidden" name="providerID" value="<?php echo $_POST['provider_id']; ?>" />
