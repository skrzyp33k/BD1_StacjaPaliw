<?php
    ...
    $query_worker = mysqli_query($conn, "SELECT * FROM Pracownicy WHERE ID_pracownika = ".$_POST['worker_id'].";");
    $query_address = mysqli_query($conn, "SELECT * FROM Adresy WHERE ID_adresu = ".$_POST['address_id'].";");

    $result_worker = mysqli_fetch_all($query_worker, MYSQLI_ASSOC);
...
<head>
    ...
    <title>GBS - Edycja pracownika</title>
...
<header class="menuBarDev p-3" style="background-color: rgb(88,101,242);">
    ...
    <div class="h1">Edytuj pracownika</div>
        <form method="POST" action="update_worker.php">
            <div class="form-group">
                <label for="positionInput">Stanowisko</label>
                <input type="text" class="form-control" id="positionInput" name="positionInput" value="<?php echo $result_worker[0]['stanowisko']; ?>" maxlength="30"/>
            </div>
            <div class="form-group">
                <label for="nameInput">Imię</label>
                <input type="text" class="form-control" id="nameInput" name="nameInput"
                    value="<?php echo $result_worker[0]['imie']; ?>" maxlength="30"/>
            </div>
            <div class="form-group">
                <label for="surnameInput">Nazwisko</label>
                <input type="text" class="form-control" id="surnameInput" name="surnameInput"
                    value="<?php echo $result_worker[0]['nazwisko']; ?>" maxlength="40"/>
            </div>
            <div class="form-group">
                <label for="phoneInput">Numer telefonu</label>
                <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="phoneInput"
                    name="phoneInput" value="<?php echo $result_worker[0]['telefon']; ?>" maxlength="12"/>
            </div>
            <div class="form-group">
                <label for="bankInput">Numer konta bankowego</label>
                <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="bankInput"
                    name="bankInput" value="<?php echo $result_worker[0]['nr_rachunku']; ?>" maxlength="26" />
            </div>
            <div class="form-group">
                <label for="salaryInput">Wyplata</label>
                <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="salaryInput"
                    name="salaryInput" value="<?php echo $result_worker[0]['wyplata']; ?>" maxlength="8"/>
            </div>
            <div class="form-group">
                <label for="streetInput">Ulica</label>
                <input type="text" class="form-control" id="streetInput" name="streetInput"
                    value="<?php echo $result_address[0]['ulica']; ?>" maxlength="50"/>
            </div>
            <div class="form-group">
                <label for="buildingInput">Numer budynku</label>
                <input type="text" class="form-control" id="buildingInput" name="buildingInput"
                    value="<?php echo $result_address[0]['nr_domu']; ?>" maxlength="5" />
            </div>
            <div class="form-group">
                <label for="flatInput">Numer mieszkania</label>
                <input type="text" onkeypress="return onlyNumberKey(event)" class="form-control" id="flatInput"
                    name="flatInput" value="<?php echo $result_address[0]['nr_mieszkania']; ?>" maxlength="5"/>
            </div>
            <div class="form-group">
                <label for="postalInput">Kod pocztowy</label>
                <input type="text" class="form-control" id="postalInput" name="postalInput"
                    value="<?php echo $result_address[0]['kod_pocztowy']; ?>" maxlength="6"/>
            </div>
            <div class="form-group">
                <label for="cityInput">Miasto</label>
                <input type="text" class="form-control" id="cityInput" name="cityInput"
                    value="<?php echo $result_address[0]['miasto']; ?>" maxlength="30"/>
            </div>
            <div class="form-group">
                <label for="countryInput">Kraj</label>
                <input type="text" class="form-control" id="countryInput" name="countryInput"
                    value="<?php echo $result_address[0]['kraj']; ?>" maxlength="20" />
            </div>
            <input type="hidden" name="workerID" value="<?php echo $_POST['worker_id']; ?>" />
