...
<header class="menuBarDev p-3" style="background-color: rgb(88,101,242);">
    <div class="container">
      ...
      <a href="../" style="height: 64px;"><img src="../img/gbs.png" class="img-fluid" style="height: 64px;"/></a>
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
          ...
          <li><a href="./faktury.php" class="nav-link px-2 text-black">Faktury</a></li>
  ...
  <?php
      ...
      $query = "SELECT Faktury.ID_zlecenia AS ID_zlecenia, Faktury.nr_faktury AS nr_faktury, Faktury.data_wystawienia AS data_wystawienia, Klienci.imie AS imie_klient, Klienci.nazwisko AS nazwisko_klient,  Pracownicy.imie AS imie_pracownik, Pracownicy.nazwisko AS nazwisko_pracownik FROM Faktury INNER JOIN Klienci ON Faktury.ID_klienta = Klienci.ID_klienta INNER JOIN Pracownicy ON Faktury.ID_pracownika = Pracownicy.ID_pracownika;";
      $result = mysqli_query($conn, $query);
      $table = mysqli_fetch_all($result, MYSQLI_ASSOC);
      foreach($table as $row)
      {
        echo '<h4>';
        echo 'ID: '.$row['nr_faktury'];
        echo ' Data: '.$row['data_wystawienia'];
        echo ' Wystawił/a: '.$row['imie_pracownik'].' '.$row['nazwisko_pracownik'];
        echo ' Dla: '.$row['imie_klient'].' '.$row['nazwisko_klient'];
        echo '</h4>';
      ...
      <form method="POST" action="insert_invoice.php">
        <div class="row">
          ...
          <label for="workerSelect">Pracownik</label>
                  ...
                  <?php
                  $query = mysqli_query($conn, 'SELECT ID_pracownika AS ID, CONCAT(imie," ",nazwisko) AS Pracownik FROM Pracownicy');
          ...
          <div class="col-sm">
          <label for="workerSelect">Klient</label>
                  <select class="form-select" aria-label="Klient" name="clientSelect" id="clientSelect">
                  <?php
                  $query = mysqli_query($conn, 'SELECT ID_klienta AS ID, CONCAT(imie," ",nazwisko) AS Klient FROM Klienci;');
